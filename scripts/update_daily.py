...
def check_rebalance_and_notify(prices, positions_dict, all_metrics):
    ...
    for name, stype, _ in [('动量轮动', 'momentum', None), ('等权再平衡', 'equal_weight', None), ('相对强弱动态配比', 'relative_strength', None), ('行业轮动(高频)', 'industry_rotation_v2', None), ('无行业轮动(小白兔)', 'no_industry', None), ('多元配置(风险平价)', 'multi_asset', None)]:
            ...
            for c, v in latest_pos.items():
                if v > 0.01:
                    if stype == 'industry_rotation':
                        ...
                        if c == '现金': display_name = '现金'
                    elif stype == 'no_industry':
                        display_name = no_industry.ETF_POOL.get(c, {}).get('name', c)
                        if c == '现金': display_name = '现金'
...
def main():
    ...
    # 3.58 无行业轮动策略（小白兔 - 28只ETF，动量+趋势破坏风控）
    print("\n  计算无行业轮动(小白兔)...")
    try:
        ni_etf_data = no_industry.fetch_all_data()
        if len(ni_etf_data) >= 5:
            ni_returns, ni_positions, ni_prices, ni_holdings = no_industry.backtest_no_industry(ni_etf_data)
            ni_metrics = no_industry.calc_metrics(ni_returns, '无行业ETF轮动(小白兔)')
            ni_metrics['strategy_type'] = 'no_industry'
            all_metrics.append(ni_metrics)
            all_positions['no_industry'] = ni_positions
            no_industry.save_strategy_json(ni_returns, ni_positions, ni_holdings, ni_metrics)
            print(f"    -> 已更新: no_industry.json")
    except Exception as e:
        print(f"    无行业轮动更新失败: {e}")
